...
function out=rgb(in)
  ...
  if isa(in,'char')
    ...
    elseif length(in)>2 & length(in)<5 & strcmpi('qb',in(1:2))
  ...
  elseif isa(in,'double')
    if size(in,1)==1 & size(in,2)==1 & abs(in)<16777216
      out=translatecolorRGB(in);
    elseif size(in,1)==1 & size(in,2)==3
      out=in;
    else
...
function out=translatecolorstring(in)
  ...
  if args.N>3
    warning(['Too many words in color description, any more than 3 will' ...
             ' be ignored']);
  end
  while(args.N<3)
  ...
  if isempty(col)
    error(['Unrecognised colour "' args.tokens{3} '", black assumed'])
    out=[0 0 0];
    return;
  end
