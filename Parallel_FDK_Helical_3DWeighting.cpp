...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    //Parallel_FDK_Helical_3DWeighting(ObjR,SO,DO,YL,ZL,DecWidth,DecHeigh,XN,YN,ZN,N_Turn,N_2pi,h,k1,delta,fpwd,ctimage,HSCoef);
    //Parallel_FDK_Helical_3DWeighting(ScanGeom,ctimage);
	...
	else 
	{
     ...
     TYPE x,y,z,Dey,Dez,touying,UU,U1,V1,Beta0,Yr,Zr,View,weight,weight1,weight2,Gama,Gama_C,m1,m2;
                 ...
                 {				
                    ...
                    if ((U>0)&&(U<YL)&&(V>0)&&(V<ZL))
                    {
                          touying = Dey*Dez*fp[((V-1)*YL+(U-1))*PN+ProjInd]
                           ...
                           weight1 = w[d1];
                           weight2 = w[d2];
                           Gama   = abs((z-h*View/(2.0*pi))/(sqrt(SO*SO-Yr*Yr)+UU));
                           if (ProjInd < s0)
                           {
                               Gama_C = abs((z-h*(View+pi)/(2.0*pi))/(sqrt(SO*SO-Yr*Yr)-UU));
                           }
                           else
                           {
                               Gama_C = abs((z-h*(View-pi)/(2.0*pi))/(sqrt(SO*SO-Yr*Yr)-UU));
